        ...
        if(!empty($_POST['titre']) AND !empty($_POST['prix']) AND !empty($_POST['description']) AND !empty($_POST['lieu'])){
            ...
            if (empty($_FILES['image1']) AND empty($_FILES['image2']) AND empty($_FILES['image3']) AND empty($_FILES['image4']) AND empty($_FILES['image5'])) {
                ...
                $numType = $reslt2['id_type'];//resultat
                    ...
                    $done = $reqAjout->execute(array($title, $desc, $price, $place, $ladate, $numVille, $numType, $leNum));
                    if ($done) {
                    //SELECTION DU DERNIER NUMERO D'ANNONCE POUR INSERTION DES IMAGES
                    $reqNumAnn = $pdo->prepare("SELECT MAX(id_ann) AS `dernier_num` FROM `annonces`");
                    $reqNumAnn->execute();
...
<html>
<head>
    <title>Bamsachine Tech - Nouvelle Annonce</title>
    <meta charset="utf-8"/>
