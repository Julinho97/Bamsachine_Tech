<?php
    ...
    if (isset($_POST['btnSubmit'])) {
        ...
        if (!empty($identification) AND !empty($password)) {
            $requeteUser = $pdo->prepare("SELECT * FROM `administration` WHERE admin_name = ? AND admin_pass = ?");/*RECHERCHE DANS LA BASE*/
            $requeteUser->execute(array($identification, $password));
            $userExiste = $requeteUser->rowCount();
            if ($userExiste == 1) {
                $adminInfo = $requeteUser->fetch();
                $_SESSION['num'] = $adminInfo['id_admin'];
                #NOM DU COOKIE __LINGOMIN = LOGIN + ADMIN
                setcookie('__lingomin', $adminInfo['id_admin'], time() + 360*24*3600, null, null, false, true);
