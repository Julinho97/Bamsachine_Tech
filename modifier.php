    ...
    if (isset($_GET['num']) AND is_numeric($_GET['num']) AND isset($_GET['numser']) AND is_numeric($_GET['numser']) AND  intval($_GET['numser']) == $id_user ) {
        $id_ann = intval($_GET['num']);
        $requeteAnn = $pdo->prepare("SELECT * FROM `annonces` WHERE id_ann = ?");
        $requeteAnn->execute(array($id_ann));
        $annInfo = $requeteAnn->fetch();
        $id_user = $annInfo['id_user'];
        if($annInfo['id_user'] == $_GET['numser']){
            ...
            $id_ville = $annInfo['id_ville'];
            $id_type = $annInfo['id_type'];
        }else{
    ...
    $reqSup = $pdo->prepare("SELECT * FROM `images` WHERE `id_ann` = ?");
    $reqSup->execute(array($id_ann));
    ?>
    <!DOCTYPE html>
    <html>
    <head>
                ...
                //VERIFICATION DES CHAMPS
                    ...
                    //VERIFIVATION DU Prix
                        ...
                        #TABLEAU DES EXTENSIONS
                            $extensions_autorisees = array('jpg', 'jpeg', 'png', 'gif', 'PNG', 'JPG', 'JPEG', 'GIF', 'MP4');
                    ...
                    //IMAGE 1
                        ...
                        //SELECTION DES NUMEROS CORRESPONDANT AUX SELECTs
                            ...
                            $reqNumVille = $pdo->prepare("SELECT `id_ville` FROM `villes` WHERE `nom_ville` = ?");
                            $reqNumVille->execute(array($nomVlle));
                            $reslt1 = $reqNumVille->fetch();
                        $numVille = $reslt1['id_ville'];//resultat

                        $reqNumType = $pdo->prepare("SELECT `id_type` FROM `type_immob` WHERE `libelle_type` = ?");
                        $reqNumType->execute(array($nomType));
                        $reslt2 = $reqNumType->fetch();
                        $numType = $reslt2['id_type'];//resultat 
                        ///MIS A JOUR DES ELEMENTS DANS LA BASE...
                        $reqUpdate = $pdo->prepare("UPDATE `annonces` SET `titre_ann` = ?, `desc_ann` = ?, `prix_ann` = ?, `lieu_ann` = ?, `id_ville` = ?, `id_type` = ? WHERE `id_ann` = ? AND `id_user` = ?");
                        $done = $reqUpdate->execute(array($title, $desc, $price, $place, $numVille, $numType, $id_ann, $leNum));
                        if ($done) {
                                 //INSERTION
                            if (isset($pathDestination1)) {
                                $reqIns1 = $pdo->prepare("INSERT INTO `images` VALUES(null, ?, ?)");
                                $reqIns1->execute(array($pathDestination1, $id_ann));
                            }
                            if (isset($pathDestination2)) {
                                $reqIns2 = $pdo->prepare("INSERT INTO `images` VALUES(null, ?, ?)");
                                $reqIns2->execute(array($pathDestination2, $id_ann));
                            }
                            if (isset($pathDestination3)) {
                                $reqIns3 = $pdo->prepare("INSERT INTO `images` VALUES(null, ?, ?)");
                                $reqIns3->execute(array($pathDestination3, $id_ann));
                            }
                            if (isset($pathDestination4)) {
                                $reqIns4 = $pdo->prepare("INSERT INTO `images` VALUES(null, ?, ?)");
                                $reqIns4->execute(array($pathDestination4, $id_ann));
                            }
                            if (isset($pathDestination5)) {
                                $reqIns5 = $pdo->prepare("INSERT INTO `images` VALUES(null, ?, ?)");
                                $reqIns5->execute(array($pathDestination5, $id_ann));
                            }
