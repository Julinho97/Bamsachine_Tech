                ...
                if(is_numeric($num_tel)){
                    if (strlen($num_tel) == 9){
                        if ($password == $password2){
                            $rInsert = $pdo->prepare("UPDATE `utilisateurs` SET `prenoms_user` = ?, `nom_user` = ?, `num_tel` = ?, `adr_mail` = ?, `password` = ? WHERE `id_user` = ?");
...
<html>
    <head>
        <title>Bamsachine Tech - Profil</title>
        <meta charset="utf-8"/>
    ...
    <body>
        ...
        <div id="corpsSignIn">
            ...
            <form id="fSignIn" action="" method="post" align="center">
                ...
                <input type="text" required placeholder="Numéro de téléphone" maxlength="10" name="num_tel" class="input" id="numTel" title="Numéro de téléphone" value="<?php if(isset($numero)) echo $numero; ?>"/>
